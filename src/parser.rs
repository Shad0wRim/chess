...
pub fn parse_move(input: &str) -> Result<Turn, ChessParseError> {
   ...
   let r#move = Move { piece, dst, flags, src, promotion };
   verify_move(r#move)
      .map_err(|e| ChessParseError { character: '=', kind: ParseErrorKind::PromotionError(e) })
}
...
fn get_piece(turn: &str) -> Result<PieceType, ChessParseError> {
   ...
   if piece_options.len() > 1 {
      Err(ChessParseError { character: piece_options[1], kind: ParseErrorKind::ExcessPieces })
   } else {
...
fn get_squares(turn: &str) -> Result<(Square, Option<Source>), ChessParseError> {
   ...
   match square_chars.chars().count() {
      0 | 1 => Err(ChessParseError { character: ' ', kind: ParseErrorKind::NeedSquare }),
      2 => Ok((
