...
impl Display for TurnError {
   fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
      match self {
         ...
         TurnError::MissingAtSquare => write!(f, "No piece that can move to the destination found at that square"),
         TurnError::MissingInLine => write!(f, "No piece that can move to the destination found in that line"),
         ...
         TurnError::CastleThroughCheck => write!(f, "Can't castle because the king would move through check"),
         TurnError::NeedCheckSpecifier => write!(f, "Need to add a `+` when giving a check"),
         TurnError::NeedCheckmateSpecifier => write!(f, "Need to add a `#` when giving checkmate"),
         TurnError::NeedCaptureSpecifier => write!(f, "Need to add a `x` when capturing a piece"),
         TurnError::RemoveCheckmateSpecifier => write!(f, "Remove `#` when not giving checkmate"),
...
impl ChessBoard {
      ...
      match turn {
         Turn::Castling(castling_type, _) => {
            ...
            match castling_type {
               CastlingType::Long => {
                  if self.is_white {
                     new_king = (Square::C1, Piece { piece: PieceType::King, is_white: true });
                     new_rook = (Square::D1, Piece { piece: PieceType::Rook, is_white: true });
                     ...
                     new_king = (
                        Square::C8,
                        Piece { piece: PieceType::King, is_white: false },
                     );
                     new_rook = (
                        Square::D8,
                        Piece { piece: PieceType::Rook, is_white: false },
                     );
               ...
               CastlingType::Short => {
                  if self.is_white {
                     new_king = (Square::G1, Piece { piece: PieceType::King, is_white: true });
                     new_rook = (Square::F1, Piece { piece: PieceType::Rook, is_white: true });
                     old_king_loc = Square::E1;
         ...
         Turn::Move(r#move) => {
            ...
            if r#move.piece == PieceType::Pawn
               && if self.is_white {
                  Line::Rank4.to_vec().contains(&r#move.dst) && Line::Rank2.to_vec().contains(&src)
               } else {
                  Line::Rank5.to_vec().contains(&r#move.dst) && Line::Rank7.to_vec().contains(&src)
               }
            {
   ...
   pub fn check_gamestate(&self) -> GameState {
      ...
      if no_moves_left && self.is_in_check(self.is_white) {
         if !self.is_white {
            return GameState::Win(Win { is_white: true, kind: WinType::Checkmate });
         } else {
            return GameState::Win(Win { is_white: false, kind: WinType::Checkmate });
         }
   ...
   fn validate_move(&self, r#move: &Move) -> Result<Source, TurnError> {
      ...
      for piece in self.find_pieces(Piece { piece: r#move.piece, is_white: self.is_white }) {
   ...
   pub fn get_minimum_move(&self, turn: &Turn) -> Turn {
      match turn {
         ...
         Turn::Move(r#move) => {
            ...
            let turn_copies = [
               Turn::Move(Move { src: None, ..*r#move }),
               Turn::Move(Move { src: Some(Source::Line(file)), ..*r#move }),
               Turn::Move(Move { src: Some(Source::Line(rank)), ..*r#move }),
            ];
   ...
   fn is_in_check(&self, is_white: bool) -> bool {
      let mut king = self.find_pieces(Piece { piece: PieceType::King, is_white });
      if let Some(king) = king.next() {
         self
            .get_player_pieces(!is_white)
   ...
   fn set(&mut self, piece: (Square, Piece)) {
   ...
   fn gen_moves(&self, full_piece: (&Square, &Piece)) -> Vec<Square> {
      let loc = full_piece.0;
      let piece = full_piece.1;
            ...
            && white_pieces
               ...
               .is_some_and(|(white_loc, _)| {
                  black_pieces
                     .iter()
                     .find(|(_, pc)| pc.piece == PieceType::Bishop)
                     .is_some_and(|(black_loc, _)| black_loc.is_light() == white_loc.is_light())
               })
   }
}
